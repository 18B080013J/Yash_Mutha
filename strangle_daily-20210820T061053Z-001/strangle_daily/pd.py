from helper import get_processed_data, get_df_from_row, pre_process
import numpy as np
import datetime
import talib
import pandas as pd
...
bnf_5 = pd.read_csv('banknifty5min.csv', index_col=0, parse_dates=True)
close = bnf_5['close']
...
bnf_5['expiry'] = np.where(bnf_5['expiry'].isnull(), expiry_bfill, bnf_5['expiry'])
    ...
    try:
        if not position:
            if row['RSI'] > 20:
                ...
                strike_price = (round(close_price/100))*100 - 200
                expiry = row['expiry']
